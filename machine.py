...
class Machine:
    def __init__(self):
        ...
        self.spin_sound.set_volume(1.5)
    ...
    def input(self):
        ...
        if (
            # translate hand movement to k_space
            keys[pygame.K_SPACE]
            and self.can_toggle
            and self.currPlayer.balance >= self.currPlayer.bet_size
        ):
            ...
            for sym in row:
                if row.count(sym) > 2:  # Potential win
                    ...
                    if len(longest_seq(possible_win)) > 2:
                        hits[horizontal.index(row) + 1] = [
                            sym,
                            longest_seq(possible_win),
                        ]
        if hits:
            ...
            for k, v in list(self.win_data.items()):
                ...
                for reel in self.reel_list:
                    if reel in animationCols and self.can_animate:
                        self.reel_list[reel].symbol_list.sprites()[
                            animationRow
                        ].fade_in = True
